...
class CostTest : FunSpec({
    ...
    test("prices match the configured model") {
        ...
        INPUT_USD_PER_1M shouldBe (1.75 plusOrMinus EPS)
        CACHED_INPUT_USD_PER_1M shouldBe (0.175 plusOrMinus EPS)
        OUTPUT_USD_PER_1M shouldBe (14.00 plusOrMinus EPS)
    }

    test("uncached input bills at the full rate") {
        turnCost(1_000_000, 0, 0) shouldBe (1.75 plusOrMinus EPS)
    }

    test("output bills at the output rate") {
        turnCost(0, 0, 1_000_000) shouldBe (14.0 plusOrMinus EPS)
    }
    ...
    test("cached tokens are a subset, not an addition") {
        ...
        turnCost(1_000_000, 1_000_000, 0) shouldBe (0.175 plusOrMinus EPS)
    ...
    test("a partly cached prompt splits across both rates") {
        // 500k uncached at $1.75 + 500k cached at $0.175
        turnCost(1_000_000, 500_000, 0) shouldBe (0.9625 plusOrMinus EPS)
    }
    ...
    test("gpt-5.3-codex still undercuts the gpt-4o rates it replaced") {
